    ...
    def process_files(self):
                    ...
                    with tempfile.TemporaryDirectory() as temp_dir:
                        ...
                        if docx_path:
                            ...
                            try:
                                ...
                                self.log_message.emit(f"繁简转换 {os.path.basename(file_path)} 时出错: {str(e)}")
                        else:
                            self.log_message.emit(f"DOC文件 {os.path.basename(file_path)} 转换失败")
        ...
        if os.path.isfile(self.input_path):
            ...
            if file_ext == '.docx':
                ...
                self.log_message.emit("错误：不支持的文件格式，仅支持doc、docx、txt、md、srt、ass、ssa、lrc、epub文件")
                return False
        ...
        # 处理文件夹
            ...
            for f in os.listdir(self.input_path):
                ...
                if file_ext in ['.doc', '.docx', '.txt', '.md', '.srt', '.ass', '.ssa', '.lrc', '.epub']:
                    supported_files.append(f)

            if not supported_files:
                self.log_message.emit("在指定文件夹中未找到支持的doc、docx、txt、md、srt、ass、ssa、lrc、epub文件")
                return False
...
class ModernUI(QMainWindow):
    ...
    def create_about_tab(self):
        ...
        <p><b>主要特性:</b></p>
        <ul>
            <li>支持Word文档、TXT文本文件、EPUB文件、字幕文件的繁体字形转换</li>
            <li>基于《通用规范汉字表》</li>
            <li>转换后保留原文档格式</li>
            <li>支持批量处理文件</li>
    ...
    def browse_input(self):
        ...
        if choice == QMessageBox.StandardButton.Yes:  # 文件夹
            ...
            paths, _ = QFileDialog.getOpenFileNames(
                self, "选择文件", "",
                "文档文件 (*.doc *.docx *.txt *.md *.srt *.ass *.ssa *.lrc *.epub);;所有文件 (*)"
            )
            if paths:
