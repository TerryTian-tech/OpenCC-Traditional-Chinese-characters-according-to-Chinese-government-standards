...
def convert_doc_to_docx(input_path, output_folder, log_callback=None, is_cancelled_callback=None):
    """
    将DOC文件转换为DOCX文件
    ...
    try:
        ...
        log(f"正在转换DOC文件: {os.path.basename(input_path)}")

        # 尝试使用Word.Application，如果失败则使用wps.Application
        word_app = None
        try:
            # 尝试使用Microsoft Word
            log("尝试使用Microsoft Word...")
            word_app = win32.gencache.EnsureDispatch('Word.Application')
        except Exception as word_error:
            log(f"Microsoft Word不可用: {str(word_error)}")
            try:
                # 尝试使用WPS
                log("尝试使用WPS Office...")
                word_app = win32.gencache.EnsureDispatch('kwps.Application')
            except Exception as wps_error:
                log(f"WPS Office也不可用: {str(wps_error)}")
                log("错误：未找到Microsoft Word或WPS Office，无法转换DOC文件")
                return False
        ...
        try:
            ...
            log(f"使用{app_type}成功转换为DOCX文件: {output_path}")
            return output_path

        except Exception as e:
            log(f"转换DOC文件时出错: {str(e)}")
            return False
        ...
        log(f"处理DOC文件 {input_path} 时出错: {str(e)}")
        return False
    finally:
